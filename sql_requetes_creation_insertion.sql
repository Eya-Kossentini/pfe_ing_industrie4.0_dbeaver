...
TRUNCATE TABLE raw.machine_condition_data;
...
SELECT COUNT(*) FROM raw.machine_condition_data; //13
...
CREATE TABLE IF NOT EXISTS raw.bookings (
    booking_id bigint ,
    workorder_id      BIGINT REFERENCES ref.work_orders(id),
    station_id        BIGINT NOT NULL REFERENCES ref.stations(id),
    failed_id         TEXT,
    serial_number_id  TEXT,
    date_of_booking   TIMESTAMPTZ,
    state             TEXT,
    mesure_id         TEXT,
    real_cycle_time   DOUBLE PRECISION,
    type              TEXT,
    created_at        TIMESTAMPTZ DEFAULT now(),
    PRIMARY KEY (booking_id, station_id)
);

truncate table raw.bookings
...
select
    m.id::bigint as  booking_id,
    CASE
        WHEN m.workorder_id IS NOT NULL THEN m.workorder_id::bigint
        ELSE NULL
    END AS workorder_id,
    m.station_id::bigint AS station_id,
    m.failed_id::text AS failed_id,
    m.serial_number_id::text AS serial_number_id,
    m.date_of_booking::timestamptz AS date_of_booking,
    m.state::text AS state,
    m.mesure_id::text AS mesure_id,
    NULLIF(NULLIF(m.real_cycle_time::text, ''), 'NULL')::numeric(10,2),
    m.type::text AS type
FROM public.bookings m 
WHERE m.date_of_booking IS NOT NULL
  AND m.station_id IS NOT NULL
  AND m.station_id::bigint IN (SELECT id FROM ref.stations)
  AND (
      m.workorder_id IS NULL
      OR m.workorder_id::bigint IN (SELECT id FROM ref.work_orders)
  )
ON CONFLICT (booking_id, station_id) DO NOTHING;

SELECT COUNT(*) FROM raw.bookings  ; //41
